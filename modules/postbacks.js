...
exports.contact_me = (sender, values) => {
    ...
    messenger.getUserInfo(sender).then(response => {
        salesforce.createLead(propertyId, response.first_name, response.last_name, sender).then(() => {
            messenger.send({text: `Thanks for your interest, ${response.last_name}. I asked the listing agent to contact you asap.`}, sender);
        });
    });
